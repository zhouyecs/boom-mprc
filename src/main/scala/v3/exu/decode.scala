...
////                               |  |  |  |  |
  ...
  io.out.is_call := (cs_is_jal && GetRd(io.inst) === BitPat("b00?01")) || (cs_is_jalr && GetRd(io.inst) === BitPat("b00?01") && GetRs1(io.inst) =/= BitPat("b00?01"))
  io.out.is_ret  := cs_is_jalr && GetRs1(io.inst) === BitPat("b00?01") && GetRd(io.inst) =/= BitPat("b00?01")
  io.out.is_pop_push := cs_is_jalr && GetRs1(io.inst) === BitPat("b00?10") && GetRs1(io.inst) === BitPat("b00?11")
